...
/obj/machinery/power/nuclear_rod/New()  // тут все пон€тно
	..()
	nrods += src
...
/obj/machinery/power/nuclear_rod/attackby(obj/item/weapon/W, mob/user)  // тут у нас реакци€ на инструменты
	if(rodtemp < 2000)
		if(!broken)
...
/obj/machinery/power/nuclear_rod/proc/check_state()   // „тобы не писать в ѕроцесс
	if (rodtemp > 4000)
...
/obj/machinery/power/nuclear_rod/proc/AddReact(var/name, var/quantity = 1)  //нужно дл€ –еакта
...
/obj/machinery/power/nuclear_rod/proc/React()  // “от самый забагованный прок, да-да
	...
	if(reactants.len)
		...
		while(possible_reactions.len)                 //а теперь все реакции пустим
			...
			if(reactants[cur_reaction.substance] > 0.01)  //ДOДБДВДuДtДuД|ДuД~ДyДu Д{ДАД|ДyДЙДuДГДДДrДp ДrДГДДДЕДБДpДuД}ДАДsДА Дr ДВДuДpД{ДИДyДР
			...
			if( reactants[cur_reaction.substance] - amount_reacting >= 0 )  //ДTДqДyДВДpДuД} ДyДx ДГДБДyДГД{Дp ДВДuДpД{ДДДpД~ДДДАДr
			...
			for(var/pr_reactant in cur_reaction.products)   //ДI ДtДАДqДpДrД|ДСДuД} ДБДВДАДtДЕД{ДДДН ДВДuДpД{ДИДyДy
				var/success = 0
		...
		for(var/prreactant in produced_reactants)
			AddReact(prreactant, produced_reactants[prreactant])  //Дp ДДДuДБДuДВДО ДrДГДu ДБДВДАДyДxДrДuДtДuД~Д~ДАДu ДyДtДuДД ДАДqДВДpДДД~ДА Дr ДВДuД{ДДДpД~ДДДН
	return 1
...
/obj/machinery/power/nuclear_rod/setupexample  //дл€ тестов
	rodtemp = 2000
	anchored = 1
	accepted_rads = 200
